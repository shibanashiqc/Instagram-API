...
class Story extends RequestCollection
{
        ...
        $reason = 'pull_to_refresh')
    {
        ...
        $this->ig->internal->batchFetch('user_feed');
   ...
   public function getUserStoryFeed(
        $userId)
    {
        $response = $this->ig->request("feed/user/{$userId}/story/")
        ...
        $feedList) 
    {
        ...
        return $this->ig->request("graphql/query/")
            ...
            ->addHeader('User-Agent', sprintf('Mozilla/5.0 (Linux; Android %s; Google) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.75 Mobile Safari/537.36', $this->ig->device->getAndroidRelease()))
            ->addParam('query_hash', 'c9c56db64beb4c9dea2d17740d0259d9')
