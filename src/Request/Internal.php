...
//            }
        ...
        $useCsrfToken = true)
    {
        ...
        if (!$prelogin) {
            $request->addCookie('ds_user', $this->ig->username)
                ->removeCookie('is_starred_enabled')
                ->removeCookie('igfl');
        }
    ...
    public function getProfileNotice()
    {
      ...
      /**
     * I don't know why we need this, but we need
     * 
     * This function sould be called 1 time per interval refresh before every following requests:
     * First feedtimeline request
     * Feedtimeline request
     * Get user feed
     * search/explore tab
     * activity tab
     * own profile tab
     ...
     */
     public function batchFetch($type) {
        $request = $this->ig->request('qp/batch_fetch/');

        $lastLoginTime = $this->ig->settings->get('last_login');
        $isSessionExpired = $lastLoginTime === null || (time() - $lastLoginTime) > $this->ig->appRefreshInterval;

         if ($this->batches[$type] === true && $isSessionExpired === false) {
            return false;
        }

        if ($isSessionExpired) {
            $this->ig->_sendLoginFlow(false, $this->ig->appRefreshInterval);
            $this->batches = [
                'feed_timeline' => false,
                'user_feed'     => false,
                'story_seen'    => false,
            ...
            if ($this->ig->randomAppRefreshInterval) {
                $this->ig->appRefreshInterval = rand(1800, 3000);
            }
        }

        switch ($type) {
            case 'feed_timeline':
                $request->addPost('surfaces_to_triggers', json_encode(
                    [
            ...
            case 'user_feed':
                $request->addPost('surfaces_to_triggers', json_encode(
                    [
                        Constants::BATCH_SURFACES[7][0] => Constants::BATCH_SURFACES[7][1],
                        Constants::BATCH_SURFACES[8][0] => Constants::BATCH_SURFACES[8][1],
                        Constants::BATCH_SURFACES[9][0] => Constants::BATCH_SURFACES[9][1],
                    ]
                ))
                ->addPost('surfaces_to_queries', json_encode(
                    [
                        Constants::BATCH_SURFACES[7][0] => Constants::BATCH_QUERY,
                        Constants::BATCH_SURFACES[8][0] => Constants::BATCH_QUERY,
                        Constants::BATCH_SURFACES[9][0] => Constants::BATCH_QUERY,
                    ]
                ));
            break;
            ...
            case 'story_seen':
                $request->addPost('surfaces_to_triggers', json_encode(
                    [
                        Constants::BATCH_SURFACES[3][0] => Constants::BATCH_SURFACES[3][1],
                    ]
                ))
                ->addPost('surfaces_to_queries', json_encode(
                    [
                        Constants::BATCH_SURFACES[3][0] => Constants::BATCH_QUERY,
                    ]
                ));
            break;
            ...
            case 'explore_tab':
                $request->addPost('surfaces_to_triggers', json_encode(
                    [
                        Constants::BATCH_SURFACES[3][0] => Constants::BATCH_SURFACES[3][1],
                        Constants::BATCH_SURFACES[5][0] => Constants::BATCH_SURFACES[5][1],
                        Constants::BATCH_SURFACES[6][0] => Constants::BATCH_SURFACES[6][1],
                    ]
                ))
                ->addPost('surfaces_to_queries', json_encode(
                    [
                        Constants::BATCH_SURFACES[3][0] => Constants::BATCH_QUERY,
                        Constants::BATCH_SURFACES[5][0] => Constants::BATCH_QUERY,
                        Constants::BATCH_SURFACES[6][0] => Constants::BATCH_QUERY,
                    ]
                ));
            break;
            ...
            case 'activity_tab':
                $request->addPost('surfaces_to_triggers', json_encode(
                    [
                        Constants::BATCH_SURFACES[1][0] => Constants::BATCH_SURFACES[1][1],
                        Constants::BATCH_SURFACES[4][0] => Constants::BATCH_SURFACES[4][1],
                    ]
                ))
                ->addPost('surfaces_to_queries', json_encode(
                    [
                        Constants::BATCH_SURFACES[1][0] => Constants::BATCH_QUERY,
                        Constants::BATCH_SURFACES[4][0] => Constants::BATCH_QUERY,
                    ]
                ));
            break;
        ...
        $request->addPost('vc_policy', 'default')
            ->addPost('_csrftoken', $this->ig->client->getToken())
            ->addPost('_uid', $this->ig->account_id)
            ->addPost('_uuid', $this->ig->uuid)
     ...
     * @return \InstagramAPI\Response\FetchQPDataResponse
     */
    public function getQPFetch($story = false)
    {
        $request = $this->ig->request('qp/batch_fetch/');
