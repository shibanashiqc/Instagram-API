...
/**
 * Nextpost Private API for Instagram v.2.3 (Android only)
 * 
 * @version NEXTPOST
...
class Instagram implements ExperimentsInterface
{
     ...
     * @var int
     */
    const EXPERIMENTS_REFRESH = 3600;
    ...
    public $client;
/**
     * Is app refresh interval random
     *
     * @var int
     */
    public $randomAppRefreshInterval = false;
    /**
    ...
    public $usertag;
    /**
     * Emulate connection speed.
     *
     * @var bool
     */
    public $emulateConnectionSpeed = false;
     ...
     * @var int
     */
    public $appRefreshInterval = 1800;
    /**
     * Constructor.
     *
...
if (!$appRefreshInterval) {
        ...
        $appRefreshInterval = 1800)
    {
        if (empty($username) || empty($password)) {
         ...
         $this->appRefreshInterval = $appRefreshInterval;
        ...
        if (!$this->isMaybeLoggedIn || $forceLogin) {
            ...
            $this->_sendLoginFlow(true, $appRefreshInterval);
             $this->emulateConnectionSpeed = true;
        ...
        $appRefreshInterval = 1800)
    {
        ...
        if ($justLoggedIn) {
            ...
            try {
                ...
                try {
                    $this->people->getFriendship($this->account_id);
                    $this->timeline->getUserFeed($this->account_id);
                    $this->highlight->getUserFeed($this->account_id);
                    $this->people->getInfoById($this->account_id, 'self_profile');
                    $this->story->getUserStoryFeed($this->account_id);
                } catch (\Exception $e) {}
